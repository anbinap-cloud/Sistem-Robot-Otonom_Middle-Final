...
class RoleBehaviorMixin:
    ...
    def nav_to_point(self, target_pos, stop_dist=0.2, speed_multiplier=1.0, max_speed=5.0):
        ...
        base_speed = 4.2 * speed_multiplier
        turn_gain = 5.0
        ...
        if speed_multiplier <= 1.0:
            if abs(angle_error) > math.pi / 4:
                ...
                base_speed *= 0.5
        wiggle = math.sin(self.sim.getSimulationTime()*3)*0.2
        v_left = base_speed - (turn_gain * angle_error) + wiggle
        v_right = base_speed + (turn_gain * angle_error) - wiggle
...
class StrikerRobot(RobotP3DX, RoleBehaviorMixin):
    ...
    def get_smart_aim(self):
        ...
        offset_y = -1.1 if gk_pos[1] > 0.0 else 1.1
...
class GoalkeeperRobot(RobotP3DX, RoleBehaviorMixin):
    ...
    def step(self):
        ...
        if self.state == "DUMB_PATROL":
            ...
            target_y = 0.0 + math.cos(t * 1.5) * 0.9 
